...
{
  uint8_t bt = SPIxx_Duplex_8bit(SPI_SD,0xFF);
  return bt;
}
...
{
  delay_init();
  delay_ms(20); //cho 20mili giay, de on dinh dien ap
}
//-----------------------------------------------
...
//					HAL_UART_Transmit(&huart2,(uint8_t*)str1,strlen(str1),0x1000);
					...
					sd_info.type = (ocr[0] & 0x40) ? CT_SD2 | CT_BLOCK : CT_SD2; // SDv2 (HC or SC)
				}
			}
		}
		else //SD version1 or MMC version3 (illegal command) Lenh khong hop le
		{
			...
			{
				sd_info.type = CT_SD1; cmd = ACMD41; // SDv1
			}
			else
			{
				sd_info.type = CT_MMC; cmd = CMD1; // MMCv3
			}
  ...
  {
    return SD_cmd(CMD0,0); //check what error
  }
//  sprintf(str1,"Type SD: 0x%02X\r\n",sdinfo.type);
//  HAL_UART_Transmit(&huart2,(uint8_t*)str1,strlen(str1),0x1000);	
  return 3; //finish init
}
//-----------------------------------------------
